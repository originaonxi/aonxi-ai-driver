...
flowchart TB
  O["Business owner outcome<br/>more calls · lower CPA · more reviews · pipeline"] --> D{{"AI Driver Orchestrator"}}

  D --> L["Local presence<br/>GBP · Apple Maps · Yelp · Bing"]
  D --> A["Ads<br/>Google · Meta · TikTok · LinkedIn"]
  D --> S["AI SDR<br/>Clay · Apollo · 6sense · Outreach"]
  D --> C["CRM + follow-up<br/>HubSpot · Salesforce · Pipedrive"]
  D --> R["Reputation<br/>Birdeye · Podium · ReviewTrackers"]
  D --> V["Voice/media<br/>ElevenLabs · Fish Audio · HeyGen"]
  D --> F["Finance/ops/forecasting<br/>Sheets · BigQuery · Airtable · DuckDB"]

  L & A & S & C & R & V & F --> M["Measure KPI"]
  M --> G{"KPI moved?"}
  G -- "no" --> D
  G -- "yes" --> DONE["Outcome delivered<br/>with evidence + cost ledger"]
  D --> H["Human co-pilot gate<br/>approve spend · approve outreach · handle risk"]
```
...
%%{init: {'look':'handDrawn','theme':'neutral'}}%%
flowchart LR
  START["Goal: dominate local search"] --> CLAIM["Claim/repair<br/>GBP · Apple · Bing · Yelp"]
  CLAIM --> SYNC["Sync hours/photos/services<br/>MapLabs · Ordering.Tools · Zoho"]
  SYNC --> REV["Generate/respond to reviews<br/>Birdeye · Podium · ReviewTrackers"]
  REV --> ADS["Run local ads<br/>Google Maps · Meta · Yelp · Nextdoor"]
  ADS --> CALLS["Track calls/forms<br/>CallRail · GA4 · Looker Studio"]
  CALLS --> LOOP["AI Driver improves weekly"]
```
...
%%{init: {'look':'handDrawn','theme':'neutral'}}%%
flowchart LR
  ICP["Define ICP"] --> INTENT["Find in-market accounts<br/>6sense · Bombora · ZoomInfo"]
  INTENT --> ENRICH["Enrich contacts<br/>Clay · Apollo · Lusha · Cognism"]
  ENRICH --> WRITE["AI research + personalization<br/>ChatGPT · Claude · Clay AI"]
  WRITE --> SEND["Sequence<br/>Outreach · Salesloft · Instantly · Smartlead"]
  SEND --> REPLY["Reply handling + meeting routing<br/>HubSpot · Chili Piper · Calendly"]
  REPLY --> KPI["Pipeline / meetings / CAC"]
```
...
%%{init: {'look':'handDrawn','theme':'neutral'}}%%
flowchart LR
  OFFER["Offer + audience"] --> CREATIVE["Generate creative<br/>Canva · Midjourney · Runway · Firefly"]
  CREATIVE --> LAUNCH["Launch ads<br/>Google Ads · Meta Ads"]
  LAUNCH --> TRACK["Track leads/calls<br/>GA4 · CallRail · HubSpot"]
  TRACK --> ANALYZE["AI analysis<br/>ChatGPT · Gemini · BigQuery"]
  ANALYZE --> OPT["Budget/creative/search-term optimization"]
  OPT --> OUT["Lower CPA / higher ROAS"]
```
...
%%{init: {'look':'handDrawn','theme':'neutral'}}%%
flowchart TB
  INTERN["AI Intern"] --> LEARN["Learn 100 tools through outcomes"]
  LEARN --> DRIVER["Operate AI Driver Orchestrator"]
  DRIVER --> TRACKS["Sales · Local · Ads · Voice · Ops · Finance · Forecasting"]
  TRACKS --> CAP["Capstone: ship live customer outcome"]
  CAP --> PROOF["Portfolio + metrics + skill.md"]
```
